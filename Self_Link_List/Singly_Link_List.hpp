...
class SinglyLinkList
{
	...
	class myLinkNode
	{
		friend class SinglyLinkList;		//将链式表声明为友类
...
public:
	SinglyLinkList();	//构造
	~SinglyLinkList();	//析构
	SinglyLinkList<T>& Insert(int k, const T& x);
	bool IsEmpty() const;
	int GetLength() const;
	bool GetData(int k, T& x);
	bool ModifyData(int k, const T& x);
	int Find(const T& x);
	SinglyLinkList<T>& DeleteByIndex(int k, T& x);
	SinglyLinkList<T>& DeleteByKey(const T& x, T& y);
...
template <class T>
SinglyLinkList<T>& SinglyLinkList<T>::Insert(int k,const T& x)
{
...
//删除算法实现
template <class T>
SinglyLinkList<T>& SinglyLinkList<T>::DeleteByIndex(int k, T& x)
{
...
//按关键字删除算法实现
template <class T>
SinglyLinkList<T>& SinglyLinkList<T>::DeleteByKey(const T& x, T& y)
{
...
//判断空表算法实现
template<class T>
bool SinglyLinkList<T>::IsEmpty() const
{
...
//求当前表长度算法实现
template <class T>
int SinglyLinkList<T>::GetLength() const
{
	int length = 0;
...
//按位置取元素算法实现
template <class T>
bool SinglyLinkList<T>::GetData(int k, T& x)
{
	myLinkNode<T>* p = head->next;
	int index = 1;
	if (k<1 || k>GetLength())
...
//按位置修改元素算法实现
template<class T>
bool SinglyLinkList<T>::ModifyData(int k, const T& x)
{
	myLinkNode<T>* p = head->next;
	int index = 1;
...
//按关键字查找算法实现
template<class T>
int SinglyLinkList<T>::Find(const T& x)
{
	myLinkNode<T>* p = head->next;
	int index = 1;
...
//实现链表打印算法
template<class T>
void SinglyLinkList<T>::OutPut(ostream& out)
{
	myLinkNode<T>* p = head->next;
...
//重载运算符<<
template<class T>
ostream& operator<<(ostream& out, SinglyLinkList<T>& x)
{
	x.OutPut(out);
	return out;
}
